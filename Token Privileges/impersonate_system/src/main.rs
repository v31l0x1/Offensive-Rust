...
use windows_sys::{
    ...
    Win32::{
        ...
        Security::{
            AdjustTokenPrivileges, Authorization::ConvertSidToStringSidA, DuplicateTokenEx,
            GetTokenInformation, ImpersonateLoggedOnUser, LookupAccountSidA, RevertToSelf,
            SE_PRIVILEGE_ENABLED, SID_NAME_USE, SecurityImpersonation, TOKEN_ADJUST_PRIVILEGES,
            TOKEN_DUPLICATE, TOKEN_PRIVILEGES, TOKEN_QUERY, TOKEN_USER, TokenImpersonation,
            TokenUser,
        },
        System::{
            Threading::{
...
fn do_system_operation(token: *mut c_void) {
    unsafe {
        let mut size = 0;
        ...
        println!(
            "[+] Current user SID: {}",
            CStr::from_ptr(sid_string as *const i8).to_str().unwrap()
        );
    }
}
